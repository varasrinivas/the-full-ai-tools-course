...
| SIM-09 | API Architecture Map | Clickable four-layer map of priorauth-api built from source ground truth (controller endpoints, 4 services, 4 repos, Postgres+seed): click a node for responsibility + key file + where the course dissects it; TRACE button lights the POST /api/requests path in sequence (policy can deny at eligibility; the score can only approve or escalate); ApprovalService carries the 0.85 ⚑ (course-canon contradiction, fixed in M-5.2). Keyboard-operable; reset; reduced-motion = static traced map. | M-3.1 |
